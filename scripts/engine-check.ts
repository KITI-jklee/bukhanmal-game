...
console.log('\n[4] 시간정지 단어 (모든 스테이지 6개 정답마다 반복, 2026-08-19 밸런스 조정)')
{
  const engine = newEngine()
  advance(0.1)
  for (let i = 0; i < 6; i += 1) answerOne(engine)
  check('6개 정답 완료', engine.snapshot().stageCorrect, 6)
  ...
  check('시간정지는 스테이지 정답 수에서 제외', stopped.stageCorrect, 6)
  ...
  if (frozen.length > 0) {
    ...
    check('정지 중 정답도 스테이지 정답 수에 포함', engine.snapshot().stageCorrect, 7)
  }
